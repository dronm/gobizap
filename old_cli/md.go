...
func NewMetadata() (*Metadata, error) {
	...
	md_file := filepath.Join(proj_dir, BUILD_DIR, MD_FILE)
	md_file_cont, err := os.ReadFile(md_file)
	if err != nil {
		return nil, fmt.Errorf("os.ReadFile() failed: %v on file: %s", err, md_file)
	}
	md := Metadata{}
	if err := xml.Unmarshal(md_file_cont, &md); err != nil {
		return nil, fmt.Errorf("xml.Unmarshal() failed: %v on file: %s", err, md_file)
	}
